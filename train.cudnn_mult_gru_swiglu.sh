...
# Compare with SiLU Mult GRU to see if SwiGLU helps
# SwiGLU formula: y = value * SiLU(gate + x)
# Same param count as standard mult gate!
set -e -x
...
echo "========================================================================"
echo "Timestamp: $(date)"
echo "Config: ~1.0B params (dim=2048, depth=27, batch=24, chunk=512)"
echo "Output: y = value * SiLU(gate + x) [same params as standard mult gate]"
