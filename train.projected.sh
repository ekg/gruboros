...
# --- Optimized ProjectedGRU Training Script ---
# Configuration:
...
# - dim=2048, H_rec=1280 (projected GRU), ~457M params (40% fewer!)
# - chunk_size=512 - 2× more gradient updates per epoch
# - batch_size=48 - conservative for 50k vocab (197× larger than byte-level)
# - grad_accum=32 - maintain reasonable effective batch (786K tokens/update/GPU)
...
#
# Performance gains vs baseline (HybridGRU):
# - 10% faster per token (495K vs 451K tok/s forward pass)
# - 40% fewer parameters (15.1M vs 25.2M per layer)
# - Smaller batch due to 197× larger vocab (50k vs 256 tokens)
...
# --- Launch Training with ProjectedGRU Architecture ---
echo "Starting ~457M parameter ProjectedGRU training on 8 GPUs."
echo "Architecture: depth=20, dim=2048, H_rec=1280, p50k_base tokenizer, 512 token chunks"
echo "Batch size: B=48, grad_accum=32 (786K tok/update/GPU, 6.3M across 8 GPUs)"
echo "Optimizations: 10% faster/token, 40% fewer params, NCCL backend, z_bias=0.0"
...
/home/erikg/micromamba/envs/mingru/bin/torchrun --nproc_per_node=$NUM_GPUS \
  ...
  \
  `# ARCHITECTURE (Deep ProjectedGRU)` \
  --dim 2048 \
  --depth 20 \
  ...
  --dropout 0.0 \
  \
  `# GRU CONFIGURATION (ProjectedGRU with H_rec=1280)` \
  --z_bias_input 0.0 \
  --z_bias_hidden 0.0 \
  --use_projected_gru \
  --h_recurrent 1280 \
  \
  `# SEQUENCES (512 tokens, 48 batch, safe for 50k vocab cross_entropy)` \
  --chunk_size 512 \
  --batch_size 48 \
  --grad_accum 32 \
  \
