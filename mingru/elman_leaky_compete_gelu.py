"""ElmanLeakyCompeteGelu - Competition gate × gelu hybrid.

Combines group softmax competition with element-wise gelu gating.
gate = group_softmax(W1 @ x) * gelu(W2 @ x)

GELU is smoother than silu around zero, which may help gradient flow.
"""
...
class ElmanLeakyCompeteGelu(nn.Module):
    """Competition × gelu hybrid gate."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.gelu_proj = nn.Linear(dim, self.dim_inner, bias=False)
    ...
    def _init_weights(self):
        ...
        nn.init.normal_(self.gelu_proj.weight, std=0.02)
    ...
    def forward(self, x, prev_hiddens=None, prev_conv_buffers=None, return_hiddens=True,
        ...
        # Hybrid gate: competition × gelu
        compete_gate = self._group_softmax(self.compete_proj(x))
        gelu_gate = F.gelu(self.gelu_proj(x))
        gate = compete_gate * gelu_gate
    ...
    def __repr__(self):
        return f"ElmanLeakyCompeteGelu(dim={self.dim}, n_groups={self.n_groups})"
