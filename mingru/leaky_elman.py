"""LeakyElman: Leaky integration RNN with haste CUDA kernels.
...
try:
    from haste_pytorch.elman_variants import LeakyElman as HasteLeakyElman
    HASTE_AVAILABLE = True
except ImportError:
    HASTE_AVAILABLE = False
    print("Warning: haste_pytorch LeakyElman not available.")
...
class LeakyElman(nn.Module):
    """
    Leaky Elman RNN with haste CUDA kernels.

    Architecture:
        1. CANDIDATE COMPUTATION:
           candidate = tanh(R @ h + Wx @ x + b)

        2. LEAKY INTEGRATION with log-space decay:
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not HASTE_AVAILABLE:
            raise RuntimeError("haste_pytorch LeakyElman is required.")
        ...
        # Haste LeakyElman (log-space discretization + input-only gate)
        self.elman = HasteLeakyElman(
    ...
    def __repr__(self):
        return f"LeakyElman(dim={self.dim}, dim_inner={self.dim_inner}, use_output_gate={self.use_output_gate})"
