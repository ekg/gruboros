"""ElmanLeakyCompeteLearnedTemp - Competition gate with learned temperature.

Adds a learned temperature parameter to the group softmax competition.
gate = group_softmax(W1 @ x / temp) * silu(W2 @ x)

The temperature is learned per-group, allowing the model to control
how "sharp" or "soft" the competition is for each group.
"""
...
class ElmanLeakyCompeteLearnedTemp(nn.Module):
    """Competition × silu hybrid gate with learned temperature."""
    ...
    def __init__(
        ...
        **kwargs
        ...
        # LEARNED TEMPERATURE per group (initialized to 1.0)
        # Use log-scale to ensure positive values
        self.log_temperature = nn.Parameter(torch.zeros(n_groups))
    ...
    def _group_softmax_with_temp(self, x):
        B, T, D = x.shape
        x = x.view(B, T, self.n_groups, self.group_size)

        # Apply learned temperature (exp to ensure positive)
        temp = torch.exp(self.log_temperature).view(1, 1, self.n_groups, 1)
    ...
    def forward(self, x, prev_hiddens=None, prev_conv_buffers=None, return_hiddens=True,
        ...
        # Hybrid gate: competition (with learned temp) × silu
        compete_gate = self._group_softmax_with_temp(self.compete_proj(x))
    ...
    def __repr__(self):
        return f"ElmanLeakyCompeteLearnedTemp(dim={self.dim}, n_groups={self.n_groups})"
