"""ElmanLeakyDiag RNN using haste's CUDA-optimized kernels.

Architecture: DIAGONAL R (like Mamba's diagonal A!) + leaky integration + input-only gate.
...
try:
    from haste_pytorch.elman_variants import ElmanLeakyDiag as HasteElmanLeakyDiag
    HASTE_AVAILABLE = True
except ImportError:
    HASTE_AVAILABLE = False
    print("Warning: haste_pytorch ElmanLeakyDiag not available.")
...
class ElmanLeakyDiag(nn.Module):
    """
    ElmanLeakyDiag: DIAGONAL R + leaky integration + input-only output gate.

    Like ElmanLeaky but with diagonal R instead of full R matrix.
    This matches Mamba's diagonal A architecture.
    ...
    Benefits:
    - D params instead of D² for recurrence (2048 vs 4M params at D=2048!)
    - No GEMM needed in recurrence - pure elementwise
    ...
    def __init__(
        ...
        **kwargs
        ...
        if not HASTE_AVAILABLE:
            raise RuntimeError("haste_pytorch ElmanLeakyDiag is required.")
        ...
        # Haste ElmanLeakyDiag (diagonal R + leaky integration)
        self.elman = HasteElmanLeakyDiag(self.dim_inner, self.dim_inner, delta_init=delta_init)
    ...
    def __repr__(self):
        return f"ElmanLeakyDiag(dim={self.dim}, dim_inner={self.dim_inner}, delta_init={self.delta_init})"
