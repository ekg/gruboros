...
# Import ElmanLeakyDiag (DIAGONAL R + leaky integration, like Mamba's diagonal A!)
try:
    from mingru.elman_leaky_diag import ElmanLeakyDiag
    print("ElmanLeakyDiag available (diagonal R + leaky, like Mamba's diagonal A!)")
except ImportError as e:
...
class minLM(Module):
    def __init__(
        ...
        *,
        ...
        elif use_elman_leaky_diag:
            # Use ElmanLeakyDiag (DIAGONAL R + leaky, like Mamba's diagonal A!)
            min_rnn_klass = ElmanLeakyDiag
            checkpoint_str = " with gradient checkpointing" if use_gradient_checkpointing else ""
            print(f"Using ElmanLeakyDiag (diagonal R + leaky{checkpoint_str}, delta_init={delta_init}) for depth={depth} model")
            rnn_kwargs = {
                'expansion_factor': expansion,
                'use_gradient_checkpointing': use_gradient_checkpointing,
                'recurrence_chunk_size': recurrence_chunk_size,
