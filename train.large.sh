...
# Large model: dim=2560, depth=24 (~2.2B params actual)
# Slower training with batch_size=16 (memory constrained)
...
OUTPUT_DIR="/mnt/nvme2n1/erikg/minlms/${TIMESTAMP}_large"

mkdir -p "$OUTPUT_DIR"

echo "================================================================"
echo "  LARGE MODEL - dim=2560, depth=24"
echo "================================================================"
...
/home/erikg/micromamba/envs/mingru/bin/torchrun \
  ...
  --use_causal_conv_gru \
  `# THROUGHPUT OPTIMIZATION: Short sequences + small batch (memory limited)` \
  --chunk_size 128 \
  --batch_size 16 \
  --grad_accum 1 \
  `# TRAINING (100K steps)` \
