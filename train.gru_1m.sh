...
# --- HybridGRU 1M Step Training (Resume from 100k) ---
# This script continues training from the 100k checkpoint to 1M steps.
#
# USAGE: Set RESUME_CHECKPOINT to your step_100000.pt path, then run:
#   ./train.gru_1m.sh
#
# With resume_step seed offset, each 100k segment sees fresh data!
#
# Configuration:
...
echo "Resuming ~794M parameter HybridGRU training from 100k → 1M steps on 8 GPUs."
echo "Architecture: depth=20, dim=2048, p50k_base tokenizer, 512 token chunks"
echo "Batch size: 114 per GPU, 7.5M tokens/update across 8 GPUs"
echo "Resume from: $RESUME_CHECKPOINT"
echo "Optimizations: NCCL backend, z_bias=0.0, seed offset prevents data repetition"
...
/home/erikg/micromamba/envs/mingru/bin/torchrun --nproc_per_node=$NUM_GPUS \
  ...
  --fused_gru \
  \
  `# SEQUENCES (512 tokens, 933K tokens per update per GPU, 7.5M across 8 GPUs)` \
  --chunk_size 512 \
  --batch_size 114 \
  --grad_accum 16 \
  \
  `# TRAINING (1M steps total, resuming from 100k)` \
  --train_steps 1000000 \
