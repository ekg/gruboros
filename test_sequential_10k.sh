#!/bin/bash
# 10K step test of SequentialTritonGRU with architecture + memory fixes
mkdir -p logs
...
OUTPUT_DIR="/tmp/test_sequential_10k_${TIMESTAMP}"
LOG_FILE="logs/test_sequential_10k_${TIMESTAMP}.log"

mkdir -p "$OUTPUT_DIR"

echo "========================================================================"
echo "SequentialTritonGRU 10K Step Test"
echo "========================================================================"
echo "Timestamp: $(date)"
echo "Config: 1.11B params (dim=2048, depth=27, batch=24, chunk=512)"
echo "Fixes: Architecture (91435a5) + Memory (33106ec)"
...
/home/erikg/micromamba/envs/mingru/bin/torchrun \
    ...
    --use_sequential_triton_gru --chunk_size 512 --batch_size 24 --grad_accum 1 \
